...
import { Search, Eye, UserCheck, RefreshCw, ShoppingCart } from "lucide-react"
import { dashboardService } from "@/lib/dashboard-services"
import type { Order, UserData } from "@/lib/types"

export function OrdersManagement() {
  const [orders, setOrders] = useState<Order[]>([])
  const [distributors, setDistributors] = useState<UserData[]>([])
  ...
  const loadData = async () => {
    try {
      ...
      const [ordersData, distributorsData] = await Promise.all([
        ...
        dashboardService.getUsersByRole("distributor"),
      ])
  ...
  const handleAssignDistributor = async (orderId: string, distributorId: string) => {
    try {
      const success = await dashboardService.assignOrderToDistributor(orderId, distributorId)
      if (success) {
        await loadData()
  ...
  const getPaymentBadge = (status: string) => {
    return status === "paid" ? (
      <Badge className="bg-green-100 text-green-800 hover:bg-green-100 border-0 font-medium">Paid</Badge>
    ) : (
      <Badge className="bg-gray-100 text-gray-800 hover:bg-gray-100 border-0 font-medium">{status}</Badge>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <Card className="bg-white border border-gray-200 shadow-sm">
        <CardHeader className="border-b border-gray-100">
          <CardTitle className="text-base font-semibold text-gray-900">Orders ({filteredOrders.length})</CardTitle>
        </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-gray-100">
                    ...
                    <TableHead className="text-gray-700 font-semibold">Distributor</TableHead>
                    ...
                    <TableRow key={order.id} className="border-gray-100 hover:bg-gray-50">
                      ...
                      <TableCell>
                        {order.assignedDistributor ? (
                          ...
                          <Select onValueChange={(distributorId) => handleAssignDistributor(order.id, distributorId)}>
                            <SelectTrigger className="w-32 border-gray-300 focus:border-pink-500 focus:ring-pink-500 bg-white text-gray-900">
                              <SelectValue placeholder="Assign" />
                            </SelectTrigger>
                            <SelectContent>
                              {distributors.map((distributor) => (
                                <SelectItem key={distributor.uid} value={distributor.uid}>
